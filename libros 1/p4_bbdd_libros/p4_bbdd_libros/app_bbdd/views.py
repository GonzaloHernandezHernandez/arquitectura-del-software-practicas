...
def libros(request):
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({"mensaje": "Libro registrado con éxito" }, status=200)
...
def crearOlistarUsuarios(request):
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({"mensaje": "Usuario registrado con éxito" }, status=200)
        except KeyError:
            return JsonResponse({"error": "Datos incompletos"}, status=400)
...
def prestamos(request):
    if request.method == 'POST':
        try:
            ...
            libro = Libro.objects.get(id=data.get('libro'))

            if Prestamos.objects.filter(libro=libro, fecha_devolucion__isnull=True).exists():
            ...
            return JsonResponse({"mensaje": "Préstamo registrado con éxito"}, status=200)
